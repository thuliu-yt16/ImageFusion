...
namespace Fusion{
    ...
    Image::Image(const char* fname, int _n){
        n = _n;
        pixel = stbi_load(fname, &w, &h, &n, _n);
    }
    void Image::save_png(const char* fname) const {
        stbi_write_png(fname, w, h, n, pixel, w * n);
    }
    void Image::save_jpg(const char* fname) const {
        stbi_write_jpg(fname, w, h, n, pixel, w * n);
    }
    ...
    Color Image::getPixel(int x, int y) const{
        if(n == 1)
            return Color(pixel[n * (w * y + x)], pixel[n * (w * y + x)], pixel[n * (w * y + x)]);
        else
